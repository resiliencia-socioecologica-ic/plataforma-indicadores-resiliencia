/*
 * Script: radar-chart.js
 *
 * Objetivo: Cria, renderiza e permite a edição de gráficos de teia (Radar Charts),
 * incluindo gráficos individuais e pares de teia (`TeiaPair`).
 *
 * Funcionamento:
 * 1. **Configurações:** Define padrões visuais (cores, fontes, etc.) e processa opções salvas.
 * 2. **Plotagem:** Desenha o gráfico de teia principal em um `<canvas>`, configurando
 * seu título, legenda, tooltips e rótulos de ponto (eixos) com lógica de truncagem
 * e destaque para valores altos/baixos. Lida com a escala e visibilidade dos dados.
 * 3. **Modal de Edição:** Gera um modal HTML para personalizar o gráfico.
 * - Exibe uma pré-visualização em tempo real das alterações de estilo.
 * - Permite editar títulos, cores de linhas, visibilidade de grupos, fontes, etc.
 * - Inclui botões para aplicar configurações (neste ou em todos os gráficos do tipo),
 * resetar para o padrão e cancelar.
 * 4. **Inicialização (`init`):**
 * - Percorre elementos HTML (`.square`) para gráficos de teia e pares de teia.
 * - Para cada um, prepara o contêiner, carrega dados e configurações salvas (se existirem).
 * - Plota o(s) gráfico(s) de teia e armazena a instância para futuras manipulações.
 * - Adiciona botões de edição que abrem o modal correspondente e integra a tabela de perguntas associada.
 */
...
const RadarChart = (() => {
    ...
    const processSavedOptions = (options, defaultTitleFromDataItem) => {
        ...
        const savedTitle = options?.title; 
        const dataItemTitle = defaultTitleFromDataItem; 
    ...
    // --- Função para configurar as opções ANINHADAS do Chart.js ---
    ...
    // Aplica settings a UMA instância 
    ...
    // Reseta settings de UMA instância 
    ...
    const openEditModal = (dataId) => {
        ...
        if (isSpecial) {
             ...
             titleAForPreview = chartInstanceA?.currentOptions?.title || originalTitleA; 
             titleBForPreview = chartInstanceB?.currentOptions?.title || originalTitleB; 
        } else { 
             ...
             originalTitleA = dataItem.title; 
             titleAForPreview = representativeInstance?.currentOptions?.title || originalTitleA; 
        ...
        // --- Configura Atualização da Preview ---
        const updatePreview = (changedInputId) => {
            const formValues = getFormValues(settingsForModal.datasetNames);
            let titleForPreviewA = titleAForPreview; 
            let titleForPreviewB = titleBForPreview; 
            ...
            if (modalChartB) { applySettings(modalChartB, formValues, true, titleForPreviewB); }
        };
        ...
        onClick('teia-apply-btn', () => {
            ...
            const applyAction = (targetKey) => {
                ...
                if (isTargetSpecial) {
                    actualTitleA = originalTitleA; 
                    actualTitleB = originalTitleB; 
                    settingsForThis = {...newSettingsFromForm}; 
                } else {
                    // Para Teia Normal
                    if (scope === 'this' || targetInstance === representativeInstance) {
                        actualTitleA = newSettingsFromForm.title; 
                    } else {
                        actualTitleA = currentOptions.title || targetInstance.config.options.plugins.title._intendedText || ''; 
                    }
        ...
        onClick('teia-reset-btn', () => {
            ...
            if(modalChartA){ resetSettings(modalChartA, isSpecial, originalTitleA); } 
            if(modalChartB){ resetSettings(modalChartB, true, originalTitleB); }      
